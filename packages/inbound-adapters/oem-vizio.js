/**
 * Map VIZIO/TPV-style OEM provider payload → canonical work order body.
 * Typical fields: PO, RMA#, Model, Serial, Problem, Ship-to, OSR Creation Date (see SCT design §3.3).
 */

const { SERVICE_TYPES } = require('@tgnd/canonical-model');

const PROVIDER_KEY = 'oem_vizio';
const DEFAULT_PAYER = 'oem_in_warranty';
const DEFAULT_SERVICE = 'osr';

/**
 * @param {object} payload - Provider body (po_number, order_number, rma_number, model, serial, problem, ship_to, ...)
 * @returns {{ canonical: object } | { error: string }}
 */
function mapOemVizioToCanonical(payload) {
  if (!payload || typeof payload !== 'object') {
    return { error: 'Payload must be an object' };
  }

  const external_id = payload.external_id ?? payload.po_number ?? payload.order_number ?? payload.rma_number ?? payload.po;
  if (!external_id || typeof external_id !== 'string') {
    return { error: 'external_id, po_number, order_number, or rma_number (string) is required' };
  }
  ...
  const canonical = {
    ...
    requested_date_start: payload.requested_date_start ?? payload.osr_creation_date ?? undefined,
...
module.exports = { mapOemVizioToCanonical, PROVIDER_KEY };
